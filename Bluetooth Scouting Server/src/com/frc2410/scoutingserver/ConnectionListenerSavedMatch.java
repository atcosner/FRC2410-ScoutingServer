...
public class ConnectionListenerSavedMatch implements Runnable
{
	MainUploadGUI m;
	
	public ConnectionListenerSavedMatch(MainUploadGUI mG)
	{
		m = mG;
	}
			...
			while(true)
			{
				...
				try 
				{
					...
					if(threadsQueue != null)
					{
						if(statusArea != null)
						{
							Thread conn = new Thread(new CommunicationThreadUpload(connection,threadsQueue,statusArea,queueIndex,statusIndex));
							//Make Sure we don't have over 6 Connections
							if(MainUploadGUI.addConnectionThread(conn) != 99)
							{
								//<6 Connections
